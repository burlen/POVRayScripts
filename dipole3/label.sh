...
do
  ...
  #s=`printf ":% 3sK:%-4s:" ${k} "${clock}" | tr ' ' '-'`
  s=`printf "%sK:%s" ${k} "${clock}"`
  n=`printf %04.f ${clock}`

  of=anno/${inpref}-${n}.png
  if [ ! -e "${of}" ]
  then
    echo "$s"

    sq-annotate.sh ${inpref}-${n}.png anno/${inpref}-${n}.png "${s}" 13 1040 0 32 w "01 Digitall" && \
    sq-overlay.sh anno/${inpref}-${n}.png ../nersc-logo-small.png anno/${inpref}-${n}.png 1227 1000 &
